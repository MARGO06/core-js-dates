...
function formatDate(date) {
  ...
  let resultTime;
  if (hour <= 12) {
    resultTime = `${hour}:${minutes}:${seconds} AM`;
  }
  if (hour > 12) {
    resultTime = `${hour - 12}:${minutes}:${seconds} PM`;
  }
  if (hour === 12) {
    resultTime = `${hour}:${minutes}:${seconds} PM`;
  }
  return `${mouth}/${day}/${year}, ${resultTime}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  let result;
  const lastPartDays = firsDay + notFullWeeks;
  if (firsDay === 0) {
    result = fullWeeks + 1;
  }
  if (firsDay === 6) {
  ...
  if (firsDay !== 6 && firsDay !== 0 && lastPartDays === 6) {
    result = fullWeeks + 1;
  }
  if (firsDay !== 6 && firsDay !== 0 && lastPartDays > 6) {
    result = fullWeeks + 2;
...
function getWeekNumberByDate(date) {
  ...
  const firstMonth = new Date(year, 0);
  const firstDay = firstMonth.getDay();
  let allDays = 0;
  let result;
  for (let i = 0; i <= month; i += 1) {
    if (i === 0) {
      const firstWeek = firstDay;
      const dayWeek = firstWeek + day;
      if (dayWeek <= 6) {
        result = 1;
      } else if (dayWeek > 6) {
        result = Math.ceil(dayWeek / 7);
      }
    }
    if (i > 0) {
      ...
      if (firstDay > 1 && firstDay < 5) {
        const wholeWeeks = Math.ceil((allDays + day) / 7);
        result = wholeWeeks;
      }
      if (firstDay === 0 || firstDay === 6 || firstDay === 5) {
        const wholeWeeks = Math.ceil((allDays + day) / 7);
        result = wholeWeeks + 1;
      }
    }
...
function getNextFridayThe13th(date) {
  ...
  for (let i = 0; i < 12; i += 1) {
    const firstDay = new Date(year, i);
    const day = firstDay.getDay();
    const thirteenthDay = (day + 12) % 7;
    if (thirteenthDay === 5) {
      result.push(i);
    }
  }
  return new Date(year, result[0], 13);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const firstDates = new Date(period.start);
  const lastDates = new Date(period.end);
  const step = countWorkDays + countOffDays;
  const firsDay = firstDates.getUTCDate();
  const lastDay = lastDates.getUTCDate();
  const result = [];
  for (let i = firsDay; i <= lastDay; i += step) {
    result.push(i);
  }
  return lastDay;
}
